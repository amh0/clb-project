...
[-68.1228259353856, -16.508230258574102]`
  ...
  // Parse input text into array of [lon, lat]
  const parseInput = (): [number, number][] => {
    try {
      const json = `[${inputText}]`;
      const arr = JSON.parse(json);
      return arr.map((v: any[]) => [v[0], v[1]]);
    } catch (e) {
  ...
  // Center of map based on first point
  ...
  const handleLineClick = (e: any) => {
    ...
    ruta.forEach((pt, i) => {
      ...
      const diff = p1.distanceTo(clickPoint) + p2.distanceTo(clickPoint) - p1.distanceTo(p2);
      if (diff < bestDiff) {
        bestDiff = diff;
        bestIdx = i;
      }
      ...
      <div className="flex-1 flex flex-col">
        <div className="p-4 flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={generarRuta} variant={"outline"} className="cursor-pointer">
              Generar ruta
            </Button>
            <Button
        ...
        <div className="flex-1">
          <MapContainer center={[center[0], center[1]]} zoom={15} scrollWheelZoom={true} minZoom={2}  maxZoom={22} className="w-full h-full">
            <ChangeMapView center={center} />
            <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" maxNativeZoom={19} maxZoom={22}/>
            {ruta.length > 0 && <Polyline positions={ruta} eventHandlers={{ click: handleLineClick }} pathOptions={{ color: "blue", weight: 8 }} />}
            {showMarkers &&
              ruta.map((pos, idx) => (
                <Marker
                  key={idx}
