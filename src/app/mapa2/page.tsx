      ...
      <div className="flex-1 flex flex-col">
        <div className="p-4 flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <Button onClick={generarRuta} className="cursor-pointer">
              Generar ruta
            </Button>
            <Button
        ...
        <div className="flex-1">
          <MapContainer center={[center[0], center[1]]} zoom={15} className="w-full h-full">
            <ChangeMapView center={center} />
            <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
            {ruta.length > 0 && <Polyline positions={ruta} eventHandlers={{ click: handleLineClick }} />}
            {showMarkers &&
              ruta.map((pos, idx) => (
                <Marker
                  key={idx}
