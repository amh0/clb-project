...
export default function LinesWithMapPage() {
  ...
  // UI
  const [sheetLinesOpen, setSheetLinesOpen] = useState(true); // listado (abajo) para mobile
  ...
  const initialCenter: [number, number] = selectedLine?.points?.[0]
    ...
    : ([-68.15, -16.5] as [number, number]); // fallback La Paz aprox
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-gray-100">
      ...
      <div className="absolute top-0 left-0 w-full">
        <NadvarPage
          title={selectedLine ? `Línea ${selectedLine.number}` : "Mostrar Rutas"}
          left={<Nadvar />}
          right={
            <Button
              variant="outline"
              onClick={() => setSheetLinesOpen(true)}
              className="rounded-full bg-bg2 text-white hover:bg-green-600 lg:hidden"
              title="Ver rutas"
            >
              <MapPinned />
            </Button>
...
<div className="hidden lg:block absolute right-6 top-[96px] z-[400]">
  <div
    className={`
      w-[380px] max-h-[70vh] rounded-[22px]
  ...
  >
    ...
    <div className="p-4 space-y-4 overflow-y-auto" style={{ maxHeight: "calc(70vh - 64px)" }}>
      ...
      {!loading && !error && lineas?.map((linea) => {
        ...
        return (
          <Card
            ...
            className={[
              "relative rounded-2xl p-3 border transition-all duration-200",
              "hover:shadow-[0_10px_24px_rgba(20,162,146,0.12)] hover:-translate-y-[1px]",
              isSelected
                ? "border-[#14a292] shadow-[0_12px_28px_rgba(20,162,146,0.20)] ring-2 ring-[#14a292]/25"
                : "border-[#14a292]/60",
            ].join(" ")}
          >
            <span
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isSelected && (
                      <span className="text-[11px] text-[#0f766e] bg-[#14a292]/10 px-2 py-[2px] rounded-full border border-[#14a292]/20 whitespace-nowrap">
                        Seleccionada
                      </span>
                    )}
                  </div>
                  <div className="text-[13px] text-gray-500 truncate">{linea.syndicate}</div>
                  {linea.zone && (
                    <div className="text-[12px] text-gray-400 truncate">{linea.zone}</div>
                  )}
                </div>
                ...
                <Button
                  variant="variant1"
                  size="sm"
                  onClick={() => setSelectedId(linea._id)}
                  className={[
      ...
      {loading && !error &&
        Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-[84px] rounded-2xl bg-gray-200/60 animate-pulse" />
        ))
      }

      {!loading && !error && (lineas?.length ?? 0) === 0 && (
      ...
      {/* SHEET INFERIOR (solo mobile/tablet) */}
      <Sheet open={sheetLinesOpen} onOpenChange={setSheetLinesOpen}>
        <SheetContent
          side="bottom"
          className="lg:hidden h-[70vh] p-0 bg-white border-t rounded-t-2xl overflow-hidden"
        >
